...
let SubmissionsService = class SubmissionsService {
    ...
    async submitQuiz(userId, lessonId, dto) {
        ...
        const gradableQuestions = lessonQuestions.filter((question) => !question.isPrivate || hasAccess);
        const submittedAnswersByQuestion = this.validateSubmittedAnswers(gradableQuestions, dto.answers);
        const evaluationsByQuestion = this.evaluateQuestions(gradableQuestions, submittedAnswersByQuestion);
        const questionsResult = this.buildQuestionsResult(gradableQuestions, submittedAnswersByQuestion, evaluationsByQuestion);
        ...
        const attempt = await this.prisma.$transaction(async (tx) => {
            ...
            if (rollingAttempts.length >= 3) {
                const attemptsToDelete = rollingAttempts.slice(0, rollingAttempts.length - 2);
    ...
    async getAttemptDetail(userId, lessonId, attemptId) {
        ...
        const evaluationsByQuestion = this.evaluateQuestions(gradableQuestions, submittedAnswersByQuestion);
        const questions = this.buildQuestionsResult(gradableQuestions, submittedAnswersByQuestion, evaluationsByQuestion);
        const totalQuestions = this.countGradableQuestions(gradableQuestions);
