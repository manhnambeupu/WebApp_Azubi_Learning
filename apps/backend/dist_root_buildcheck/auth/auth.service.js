...
let AuthService = class AuthService {
    ...
    async login(email, password) {
        ...
        if (!user.password) {
            throw new common_1.UnauthorizedException('Tài khoản này sử dụng đăng nhập bằng Google/Facebook. Vui lòng dùng nút đăng nhập tương ứng.');
        }
        const now = new Date();
    ...
    async validateOAuthLogin(email, fullName, authProvider, providerId) {
        const user = await this.usersService.findOrCreateByProvider(email, fullName, authProvider, providerId);
        const { accessToken, refreshToken } = await this.generateTokens(user);
        const refreshTokenHash = await bcrypt_1.default.hash(refreshToken, 12);
        await this.prisma.user.update({
            where: { id: user.id },
